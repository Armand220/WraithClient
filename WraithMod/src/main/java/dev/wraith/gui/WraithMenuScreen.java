...
public class WraithMenuScreen extends Screen {
    ...
    private static final int CARD_GX = 8;
    private static final int CARD_GY = 8;
    private static final int CARD_H  = 136;

    // ── Palette ────────────────────────────────────────────────────────────────
    private static final int C_TOP_BAR   = 0xEE070707;
    private static final int C_TAB_STRIP = 0xE8101010;
    private static final int C_BODY      = 0xC00D0D0D;
    ...
    private static final int C_CLIP_BODY = 0xFF0D0D0D;

    private static final int C_BTN_ON    = 0xFF1B5E20;
    private static final int C_BTN_ON_H  = 0xFF2E7D32;
    private static final int C_BTN_ON_T  = 0xFF81C784;
    ...
    public WraithMenuScreen() { super(Text.literal("Wraith")); }
    ...
    public void render(DrawContext ctx, int mx, int my, float delta) {
        ...
        cardW = (gridW - (COLS - 1) * CARD_GX) / COLS;
        ...
        // ── Top bar content ────────────────────────────────────────────────────
        ctx.drawText(textRenderer, "WRAITH", px + 16, py + (TOP_H - 9) / 2, C_T_NAME, false);

        // Layout editor button
        String layLabel = "Edit Layout";
        int layW = textRenderer.getWidth(layLabel) + 16;
        ...
        ctx.drawText(textRenderer, layLabel, layX + 8, layY + (20 - 9) / 2,
        ...
        int xtw = textRenderer.getWidth("x");
        ctx.drawText(textRenderer, "x", clX + (20 - xtw) / 2, clY + (20 - 9) / 2,
        ...
        for (Category c : Category.values()) {
            ...
            int lw = textRenderer.getWidth(c.label);
            int tw = lw + 22;
            ...
            ctx.drawText(textRenderer, c.label, tpX + 11, tpY + (tpH - 9) / 2, tc, false);
        ...
        // Version stamp
        String ver = "v1.0";
        ctx.drawText(textRenderer, ver, px + W - textRenderer.getWidth(ver) - 14,
            py + (TOP_H - 9) / 2, C_T_GHOST, false);
    }

    // ── Card ───────────────────────────────────────────────────────────────────

    private void drawCard(DrawContext ctx, Feature f,
        ...
        // Feature name (top)
        ctx.drawText(textRenderer, f.getName(), x + 10, y + 11,
        ...
        // Abbreviation — centered horizontally, vertically centered in preview area
        String icn = icon(id);
        int itw = textRenderer.getWidth(icn);
        ctx.drawText(textRenderer, icn,
            pvX + (pvW - itw) / 2, pvY + (pvH - 9) / 2 - 4,
            recolor(lerpColor(0xFF4A4A4A, 0xFF999999, ta), fadeA), false);

        // Short description at bottom of preview
        String dsc = desc(id);
        if (!dsc.isEmpty()) {
            int dtw = textRenderer.getWidth(dsc);
            String display = dtw > pvW - 12
                ? dsc.substring(0, Math.max(0, (pvW - 24) / 6)) + "..."
                : dsc;
            int ddtw = textRenderer.getWidth(display);
            ctx.drawText(textRenderer, display, pvX + (pvW - ddtw) / 2, pvY + pvH - 12,
        ...
        int btw = textRenderer.getWidth(btnLabel);
        ctx.drawText(textRenderer, btnLabel, btnX + (btnW - btw) / 2, btnY + (btnH - 9) / 2,
            recolor(btnTxt, fadeA), false);
    }

    // ── Corner-clip helpers ─────────────────────────────────────────────────────

    private static void roundCorners(DrawContext ctx, int x, int y, int w, int h, int cc) {
    ...
    public boolean mouseClicked(Click click, boolean handled) {
        ...
        // Close
        int clX = px + W - 34, clY = py + (TOP_H - 20) / 2;
        if (mx >= clX && mx < clX + 20 && my >= clY && my < clY + 20) { close(); return true; }

        // Edit Layout
        String layLabel = "Edit Layout";
        int layW = textRenderer.getWidth(layLabel) + 16;
        int layX = clX - layW - 8;
        if (mx >= layX && mx < layX + layW && my >= clY && my < clY + 20) {
            client.setScreen(new WraithHudEditorScreen());
            return true;
        }
        ...
        int tpY = py + TOP_H + 9, tpH = TAB_H - 18, tpX = px + 16;
        for (Category c : Category.values()) {
            int tw = textRenderer.getWidth(c.label) + 22;
