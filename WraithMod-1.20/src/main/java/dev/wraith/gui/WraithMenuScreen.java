...
public class WraithMenuScreen extends Screen {
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.drawCenteredTextWithShadow(textRenderer, "W R A I T H", width / 2, 18, 0xFFD0D0D0);
        ctx.drawCenteredTextWithShadow(textRenderer, "Right Shift to close", width / 2, 30, 0xFF666666);

        ctx.fill(width / 2 - 100, 40, width / 2 + 100, 41, 0xFF333333);
        ...
        for (int ci = 0; ci < cats.length; ci++) {
            ...
            ctx.drawCenteredTextWithShadow(textRenderer, cat.label, cx + CARD_W / 2, cy, 0xFF888888);
            cy += 13;
            ...
            for (Feature f : features) {
                ...
                ctx.fill(cx,     cy,     cx + CARD_W,     cy + CARD_H,     border);
                ctx.fill(cx + 1, cy + 1, cx + CARD_W - 1, cy + CARD_H - 1, bg);

                ctx.fill(cx + 10, cy + CARD_H / 2 - 2, cx + 14, cy + CARD_H / 2 + 2, dot);
                ctx.drawTextWithShadow(textRenderer, f.getName(), cx + 20, cy + (CARD_H - 9) / 2, 0xFFEEEEEE);

                if (enabled) {
                    String on = "ON";
                    int bx = cx + CARD_W - textRenderer.getWidth(on) - 8;
                    ctx.drawTextWithShadow(textRenderer, on, bx, cy + (CARD_H - 9) / 2, 0xFF55FF55);
                }
